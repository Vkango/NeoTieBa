...
const userInfo = ref<UserBarInfo>({
    is_like: false,
    level_id: 0,
    level_name: '',
    cur_score: 0,
...
const barRule = ref<BarRule>({
    title: '',
    preface: '',
    rules: []
})

const bawuList = ref<BawuMember[]>([])
...
const groupedBawu = computed(() => {
    const groups: Record<string, BawuGroup> = {}
...
const loadData = async () => {
    ...
    try {
        ...
        try {
            ...
            console.log('用户未登录:', error instanceof Error ? error.message : String(error))
            isLoggedIn.value = false
        }

        // 先使用基础的吧信息初始化（防止获取详情失败导致空页面）
        barInfo.value = {
            id: forumId,
            name: props.forumData.name,
            avatar: props.forumData.avatar,
            slogan: props.forumData.slogan || '',
            description: '',
        ...
        // 获取吧扩展信息（非必须，失败不影响基本显示）
        try {
            const forumDetail = await n.getForumDetail(forumId, bduss, stoken) as ForumDetailResponse

            if (forumDetail && forumDetail.error_code === '0' && forumDetail.forum_info) {
                ...
                barInfo.value = {
                    ...
                    member_num: info.member_count || barInfo.value.member_num,
                    post_num: info.post_count || barInfo.value.post_num,
                    thread_num: info.thread_count || barInfo.value.thread_num
                }
            }
        } catch (error) {
            console.error('获取吧详情信息失败:', error)
            // 继续执行，使用已初始化的基本信息
        }
        ...
        if (bduss) {
            try {
                // 获取签到信息
                const signInfo = await n.getUserSign(forumId, bduss, stoken) as UserSignResponse
                if (signInfo && signInfo.error_code == 0 && signInfo.data?.forum?.[0]) {
                    const userSignInfo = signInfo.data.forum[0].sign_in_info.user_info

                    try {
                        // 获取用户等级信息
                        const levelInfo = await n.getUserForumLevelInfo(forumId, bduss, stoken) as UserForumLevelInfoResponse
                        console.log("cc", levelInfo)
                        if (levelInfo && levelInfo.error_code === 0 && levelInfo.data) {
        ...
        // 获取吧规（非必须，失败不影响）
        try {
            const ruleData = await n.getForumRule(forumId, bduss, stoken) as ForumRuleResponse
            if (ruleData && ruleData.error_code === '0' && ruleData.forum_rule_id) {
                barRule.value = {

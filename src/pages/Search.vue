...
<template>
  ...
  <div v-if="!isLoading" class="container1">
    ...
    <div class="navi-buttons">
      <RippleButton class="button-nostyle" :class="{ 'selected': searchType == 0 }" @click="searchType = 0">搜吧</RippleButton>
      <RippleButton class="button-nostyle" :class="{ 'selected': searchType == 1 }" @click="searchType = 1">搜贴</RippleButton>
      <RippleButton class="button-nostyle" :class="{ 'selected': searchType == 2 }" @click="searchType = 2">搜人</RippleButton>
    </div>
    <div class="result" v-if="searchType == 0">
      ...
      <div v-if="searchResult.fuzzyMatch.avatar != undefined">
        <Tag>相关匹配</Tag>
        <div style="margin-top: 10px; display: flex; gap: 10px; flex-direction: row; flex-wrap: wrap;">
          <button class="bar-button" v-for="item in searchResult.fuzzyMatch" @click="emit('BarNameClicked', item.forum_name)">
    ...
    <div class="result" v-if="searchType == 2">
      ...
      <div v-if="searchResult.fuzzyMatch[0].id != undefined">
        <Tag>相关匹配</Tag>
        <div style="margin-top: 10px; display: flex; gap: 10px; flex-direction: row; flex-wrap: wrap;">
          <button class="bar-button" v-for="item in searchResult.fuzzyMatch" @click="emit('UserNameClicked', item.id)">

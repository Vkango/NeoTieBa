...
import { getUserList, remove_user, switch_user, add_user } from '../user-manage';
...
const handleQRLoginSuccess = async (userData) => {
    ...
    await processLoginSuccess(completeUserData, '扫码登录成功');
    currentView.value = 'main';
};

const processLoginSuccess = async (userData, loginType = '登录成功') => {
    try {
...
const deleteUser = async () => {
    if (deleteConfirmUser.value) {
        await remove_user(deleteConfirmUser.value);
        await loadUsers();
...
const handleBrowserLoginSuccess = async (cookieData) => {
    try {
        let bduss, stoken;
